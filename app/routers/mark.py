...
def mark_multi_obj(df, assessment_id):
    ...
    for question in questions:
        ...
        for user in users:
            ...
            row = {'assessment_id':assessment_id,'question_id':question, 'student_id':user, 'score':is_correct*score}
...
def mark_multiple_sub(df, assessment_id):
    ...
    for question in questions:
        ...
        for user in users:
            ...
            row = {'assessment_id':assessment_id,'question_id':question, 'student_id':user, 'score':sum_score*mark/num_answer}
            new_df = pd.DataFrame([row])
            result_df = pd.concat([result_df, new_df], axis=0, ignore_index=True)
    return result_df
...
def mark_multiple_nlp(df: pd.DataFrame, assessment_id):
    nlp_df:pd.DataFrame = pd.DataFrame(columns = ['assessment_id','question_id', 'student_id', 'ref_answer', 'stu_answer','num_answer','mark', 'score'])
    questions = df["question_id"].unique().tolist()
    users = df["student_id"].unique().tolist()
    for question_id in questions:
        ref_answers = df[df['question_id'] == question_id]["ref_answer"].unique().tolist()
        num_answer = int(df[df['question_id'] == question_id]['num_answer'].iloc[0])
        student_answers = df[df['question_id'] == question_id][['stu_answer', 'student_id', 'mark']]
...
def process_multi_nlp(df:pd.DataFrame, assessment_id):
    score_df:pd.DataFrame = pd.DataFrame(columns = ['assessment_id','question_id', 'student_id', 'score'])
    ...
    for question_id in questions:
        ...
        for user in users:
            score = df[(df['question_id']== question_id) & (df['student_id'] == user)].sort_values(by="score", ascending=False)['score'][:num_answer].sum()
...
def mark_single_nlp(df: pd.DataFrame, assessment_id):
    score_df:pd.DataFrame = pd.DataFrame(columns = ['assessment_id','question_id', 'student_id', 'ref_answer', 'stu_answer', 'mark', 'score'])
    questions = df["question_id"].unique().tolist()
    users = df["student_id"].unique().tolist()
    for question_id in questions:
    ...
    score_df = score_df[['assessment_id','question_id', 'student_id','score']]
    return score_df
...
def mark_assessment(id:int, db:Session=Depends(get_db),
    ...
    single_nlp_scores = mark_single_nlp(nlp_single_df, assessment_id=id)
    ...
    total_df = score_df.groupby(['assessment_id', 'student_id'], as_index = False)['score'].sum()
    assessment_mark = assessment_detail.total_mark
    total_df['total'] = ((total_df['score'] * assessment_mark)/total_question_mark).round(2)
